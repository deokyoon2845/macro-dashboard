...
SECTORS = ["반도체","방산","증권·금융","우주항공","로봇·자동화","2차전지","바이오",
           "IT·소프트웨어","엔터·미디어","자동차","화학","철강·소재","건설","유틸리티","소비재",
           "미용","에너지","기타"]
...
st.markdown(
    ...
    """&family=JetBrains+Mono:wght@400;500;600&family=Gowun+Batang:wght@400;700&display=swap" """
    """rel="stylesheet">""" +
    f"""<style>
html,body,[class*="css"]{{
...
.stButton>button{{
  ...
  font-family:{FF}!important;font-size:12px!important;
...
[data-testid="stRadio"]>div>label[data-selected="true"]{{
  background:{B5}!important;color:#FFFFFF!important;border-color:{B5}!important}}
...
[data-testid="collapsedControl"]{{
  ...
  width:2.4rem!important;top:.8rem!important;
  box-shadow:4px 0 14px rgba(56,139,253,.35)!important;transition:all .2s!important}}
[data-testid="collapsedControl"]:hover{{
  background:{C2}!important;box-shadow:4px 0 20px rgba(56,139,253,.5)!important}}
[data-testid="collapsedControl"] svg{{color:{B5}!important;fill:{B5}!important}}
</style>""",
    unsafe_allow_html=True
)

# ── 사이드바 플로팅 열기 버튼 (SyntaxError 방지: 문자열 변수 분리) ──
_s = (
...
_js = ("(function(){"
       ...
       "var b=document.getElementById('pf-sb-btn');"
       "if(!b)return;"
       ...
       "})();")
st.markdown(
    f'<div id="pf-sb-btn" style="{_s}" onclick="{_oc}">☰ 메뉴</div>'
    f'<script>{_js}</script>',
    unsafe_allow_html=True
)
...
def compute_pos(item, prices, usdkrw):
    ...
    base = {"name":item["name"],"ticker":ticker,"sector":item.get("sector","기타"),
            "market":item.get("market","기타"),"currency":item.get("currency","KRW"),
            "account":item.get("account","일반"),
            "qty":qty,"avg_cost":avg,"id":item.get("id",""),"lots":lots}
    if cur is None:
...
def compute_daily_pf(pf_list, prices, mdf):
    ...
    for d in all_dates:
        ...
        if any_pos: rows.append({"date":d,"value":tv,"cost":tc,
...
# 계좌 필터
portfolio_view = (portfolio if sel_account is None
                  else [p for p in portfolio if p.get("account","일반")==sel_account])
positions = ([p for p in positions_all] if sel_account is None
             else [p for p in positions_all if p["account"]==sel_account])
...
if sel_account is None and accts_in_use:
    ...
    for col, acct in zip(acct_cols, accts_in_use):
        ...
        a_pct = (a_pnl/sum(p["cost_krw"] for p in holds)*100
                 if sum(p.get("cost_krw",0) for p in holds)>0 else 0)
...
<div style="background:{CARD};border:1px solid {BORD};border-top:3px solid {clr};
  ...
  <div style="font-size:20px;font-weight:800;color:{TXT};
    font-family:'JetBrains Mono',monospace">
    {a_val/1e6:.1f}<span style="font-size:11px;color:{MUT}">백만원</span>
  </div>
  <div style="font-size:10px;color:{pclr};font-weight:600;
    font-family:'JetBrains Mono',monospace;margin-top:3px">
    {sym}{abs(a_pnl):,.0f}원 ({a_pct:+.2f}%)</div>
...
<div style="padding:16px 0 10px">
  <div style="font-size:12px;color:{SUB};font-weight:500;letter-spacing:.05em;
    text-transform:uppercase;margin-bottom:4px">
    포트폴리오 {f'· {acct_label}' if acct_label else ''}
  </div>
                ...
                if st.form_submit_button("등록", type="primary"):
                    if n_name and n_ticker:
                        new = {"id":str(uuid.uuid4())[:8],"name":n_name,"ticker":n_ticker,
                               ...
                               "currency":n_currency,"account":n_acct,"lots":[]}
                        if n_qty>0 and n_price>0:
                            new["lots"].append({"date":str(n_date),"qty":n_qty,
                                                "price":n_price,"type":"buy"})
                    ...
                    for p in portfolio:
                        if p["id"]==tid:
                            p.setdefault("lots",[]).append(
                                {"date":str(a_date),"qty":a_qty,"price":a_price,"type":"buy"})
                            break
                    save_portfolio(portfolio)
                    st.cache_data.clear(); st.success("추가!"); st.rerun()

        elif manage_mode=="종목 삭제" and portfolio:
            to_del = st.selectbox("삭제할 종목",
                                  ["선택..."]+[f"{p['name']} [{p.get('account','일반')}]"
                                               for p in portfolio], key="del_sel")
...
if positions:
    ...
    for i,pos in enumerate(sorted(positions, key=lambda x:x["value_krw"], reverse=True)[:8]):
        ...
        val_str=(f"{pos['value_krw']/1e8:.2f}억" if pos["value_krw"]>=1e8
                 else f"{pos['value_krw']/1e4:.0f}만")
        ticker_items += f"""
<div style="display:flex;flex-direction:column;gap:3px;padding:10px 16px;
  ...
  <div style="display:flex;align-items:center;gap:6px">
    ...
    <span style="font-size:11px;font-weight:600;color:{TXT};overflow:hidden;
      text-overflow:ellipsis;white-space:nowrap;max-width:120px">{pos['name']}</span>
  </div>
  <div style="display:flex;align-items:baseline;justify-content:space-between">
    <span style="font-size:13px;font-weight:700;color:{TXT};
      font-family:'JetBrains Mono',monospace">{val_str}</span>
    <span style="font-size:11px;font-weight:600;color:{pclr};
      font-family:'JetBrains Mono',monospace">{sign}{pct:.2f}%</span>
  </div>
</div>"""
    st.markdown(f"""
<div style="display:flex;gap:8px;overflow-x:auto;padding:4px 0 12px;
  scrollbar-width:thin;scrollbar-color:{BORD} transparent">
  {ticker_items}
</div>""", unsafe_allow_html=True)
...
<div style="background:{CARD};border:1px solid {BORD};border-radius:12px;
  ...
  <div style="font-size:18px;font-weight:600;color:{TXT};margin-bottom:8px">
    {"이 계좌에 보유 종목 없음" if sel_account else "보유 종목 없음"}</div>
...
<div style="background:{CARD};border:1px solid {BORD};border-radius:12px;
  ...
  <div style="font-size:12px;color:{SUB};font-weight:500;text-transform:uppercase;
    letter-spacing:.05em;margin-bottom:10px">총 평가금액</div>
  <div style="font-size:36px;font-weight:800;color:{TXT};
  ...
  <div style="margin-top:10px;font-size:11px;color:{SUB};
    font-family:'JetBrains Mono',monospace">
    ...
    for i, ((_,row),clr) in enumerate(zip(df_sorted_val.iterrows(), HOLD_COLORS)):
        ...
        segments += (f'<div style="width:{w:.2f}%;background:{clr};height:100%;'
                     f'{"border-radius:"+rr+";" if rr else ""}"></div>')
...
<div style="display:flex;align-items:center;justify-content:space-between;
  padding:7px 0;border-bottom:1px solid {BORD};gap:8px">
  <div style="display:flex;align-items:center;gap:8px;flex:1;min-width:0">
    <span style="width:8px;height:8px;border-radius:50%;background:{clr};flex-shrink:0"></span>
    <span style="font-size:12px;color:{TXT};font-weight:500;overflow:hidden;
      text-overflow:ellipsis;white-space:nowrap">{row['name']}</span>
  </div>
  <div style="display:flex;align-items:center;gap:12px;flex-shrink:0">
    ...
    <span style="font-size:12px;font-weight:600;color:{TXT};
      font-family:'JetBrains Mono',monospace;min-width:70px;text-align:right">
      {row['value_krw']:,.0f}</span>
    <span style="font-size:11px;font-weight:600;color:{pclr};
      font-family:'JetBrains Mono',monospace;min-width:56px;text-align:right">
      {sp}{row['pnl_pct']:.2f}%</span>
  </div>
</div>"""
...
<div style="background:{CARD};border:1px solid {BORD};border-radius:12px;
  ...
  <div style="font-size:11px;color:{SUB};font-weight:500;text-transform:uppercase;
    letter-spacing:.05em;margin-bottom:12px">자산 구성</div>
  <div style="display:flex;height:8px;border-radius:4px;overflow:hidden;
    background:{C2};margin-bottom:14px;gap:1px">
    {segments}
  </div>
...
with right:
    ...
    if not filtered.empty:
        ...
        fig.add_trace(go.Scatter(
            ...
            fill="tonexty", fillcolor="rgba(56,139,253,.12)",
            hovertemplate="<b>포트폴리오</b> %{y:,.0f}원<extra></extra>"),
            secondary_y=False)
        fig.add_trace(go.Scatter(
        ...
        if "cost" in filtered.columns:
            ...
            if cost_last>0:
                fig.add_hline(y=cost_last, line_dash="dash", line_color=SUB,
                    line_width=1, annotation_text=f"매입 {cost_last/1e8:.2f}억",
                    annotation_font_color=SUB, annotation_font_size=9)

        if not fx_series.empty:
            fig.add_trace(go.Scatter(
                x=fx_series["date"], y=fx_series["value"], name="환율",
                line=dict(color=B3,width=1.5,dash="dot"),
                hovertemplate="<b>USD/KRW</b> %{y:,.0f}<extra></extra>"),
                secondary_y=True)
        ...
        fig.update_layout(
            ...
            legend=dict(orientation="h",y=1.05,x=0,
                font=dict(size=10,color=SUB),bgcolor="rgba(0,0,0,0)"),
            hovermode="x unified",
            hoverlabel=dict(bgcolor=C3,bordercolor=BORD,
                font=dict(family="JetBrains Mono",size=11,color=TXT)),
            xaxis=dict(showgrid=False,zeroline=False,showline=False,
                tickfont=dict(size=9,color=MUT),tickcolor=BORD),
            yaxis=dict(showgrid=True,gridcolor=G,zeroline=False,showline=False,
                tickfont=dict(size=9,color=MUT),tickformat=",.0f",side="right"),
            yaxis2=dict(showgrid=False,zeroline=False,showline=False,
                tickfont=dict(size=9,color=B3),side="left"))
        ...
        mdd_pct=(min_v/max_v-1)*100; chg_pct=(last_v/first_v-1)*100
        metrics=[
            ("최고점",f"{max_v/1e8:.3f}억원",max_d,B5),
            ("최저점",f"{min_v/1e8:.3f}억원",min_d,DN),
            ("기간 수익",f"{chg_pct:+.2f}%","기간내",UP if chg_pct>=0 else DN),
...
<div style="background:{C2};border:1px solid {BORD};border-radius:8px;padding:12px 14px">
  ...
  <div style="font-size:15px;font-weight:700;color:{clr};
    font-family:'JetBrains Mono',monospace;line-height:1.2">{val}</div>
  <div style="font-size:9px;color:{MUT};margin-top:3px">{sub_}</div>
</div>""", unsafe_allow_html=True)
    else:
        st.markdown(f"""
<div style="background:{CARD};border:1px solid {BORD};border-radius:12px;
  padding:40px;text-align:center;height:340px;
...
if sel_account is None and accts_in_use and not prices.empty:
    st.markdown(
        f'<div style="font-size:14px;font-weight:600;color:{SUB};'
        f'margin:1.2rem 0 .5rem">💼 계좌별 평가금액 추이</div>',
        unsafe_allow_html=True)

    fig_bal = go.Figure()
    for acct in accts_in_use:
        pf_acct = [p for p in portfolio if p.get("account","일반")==acct]
        if not pf_acct: continue
        h = compute_daily_pf(pf_acct, prices, mdf)
        if h.empty: continue
        fig_bal.add_trace(go.Scatter(
            x=h["date"], y=h["value"]/1e6,
            name=ACCT_LABELS.get(acct,acct),
            mode="lines", stackgroup="one",
            line=dict(width=1.5, color=ACCT_COLORS.get(acct,B5)),
            hovertemplate=f"<b>{ACCT_LABELS.get(acct,acct)}</b> %{{y:,.1f}}백만<extra></extra>"))
    ...
    fig_bal.update_layout(
        ...
        xaxis=dict(showgrid=False,zeroline=False,tickfont=dict(size=9,color=MUT)),
        yaxis=dict(showgrid=True,gridcolor=G,zeroline=False,
            tickfont=dict(size=9,color=MUT),ticksuffix="M",side="right"))
...
<div style="background:{CARD};border:1px solid {BORD};border-radius:10px;padding:16px 18px">
  <div style="font-size:10px;color:{SUB};font-weight:500;text-transform:uppercase;
    letter-spacing:.04em;margin-bottom:6px">{lbl}</div>
  <div style="font-size:20px;font-weight:800;color:{TXT};
...
# 거래 입력 도우미 (JSON 생성)
# ══════════════════════════════════════════════════════════════
with st.expander("➕ 거래 입력 (JSON 스니펫 생성 → portfolio.json에 복붙)"):
    st.caption("Streamlit Cloud는 파일 직접 저장 불가 → 생성된 JSON을 GitHub portfolio.json에 복붙하세요")
    ic1,ic2,ic3 = st.columns(3)
    with ic1:
        in_name   = st.text_input("종목명", key="in_name")
        in_ticker = st.text_input("티커",   key="in_ticker", placeholder="005930.KS")
    with ic2:
        in_acct = st.selectbox("계좌",    ALL_ACCOUNTS, key="in_acct")
        in_cur  = st.selectbox("통화",    ["KRW","USD"], key="in_cur")
    with ic3:
        in_type   = st.selectbox("구분",  ["buy","sell"], key="in_type")
        in_sector = st.text_input("섹터", key="in_sector", placeholder="반도체")
    ic4,ic5,ic6 = st.columns(3)
    with ic4: in_date  = st.date_input("거래일",  key="in_date")
    with ic5: in_qty   = st.number_input("수량",   min_value=0.0, step=1.0,   key="in_qty")
    with ic6: in_price = st.number_input("단가",   min_value=0.0, step=100.0, key="in_price")
    if st.button("JSON 생성", key="gen_json"):
        snippet = {"name":in_name,"ticker":in_ticker,"currency":in_cur,
                   "sector":in_sector,"account":in_acct,
                   "lots":[{"date":str(in_date),"qty":in_qty,
                             "price":in_price,"type":in_type}]}
        st.code(json.dumps(snippet, ensure_ascii=False, indent=2), language="json")
        st.caption("↑ 새 종목이면 이 객체를 portfolio.json에 추가, 기존 종목이면 lots 항목만 추가")
...
with t1:
    ...
    def fmt_row(r,i):
        ...
        sp   = "▲" if r["pnl_pct"]>=0 else "▼"
        sd   = "▲" if r["daily_pct"]>=0 else "▼"
...
<td style="padding:.7rem 1rem;text-align:center">
  <span style="background:{a_clr}22;color:{a_clr};padding:2px 7px;border-radius:5px;
    font-size:9px;font-weight:700">{r.get('account','일반')}</span>
</td>
<td style="padding:.7rem 1rem;text-align:right;font-family:'JetBrains Mono',monospace;
  font-size:12px;color:{TXT}">{r['qty']:,.0f}</td>
<td style="padding:.7rem 1rem;text-align:right;font-family:'JetBrains Mono',monospace;
  font-size:12px;color:{TXT}">{r['avg_cost']:,.2f}</td>
...
<td style="padding:.7rem 1rem;text-align:right">
  <div style="font-family:'JetBrains Mono',monospace;font-size:13px;font-weight:700;
    color:{TXT}">{r['value_krw']:,.0f}</div>
  <div style="font-size:10px;color:{SUB}">{wt:.1f}%</div>
</td>
<td style="padding:.7rem 1rem;text-align:right">
  <div style="font-family:'JetBrains Mono',monospace;font-size:13px;font-weight:700;
    color:{pclr}">{r['pnl_krw']:+,.0f}</div>
  <div style="font-size:10px;color:{pclr}">{sp}{abs(r['pnl_pct']):.2f}%</div>
    ...
    TH = (f"padding:.6rem 1rem;text-align:left;font-size:10px;color:{MUT};"
...
def news_card_new(news, accent=B5):
    ...
    if score is not None:
        ...
        badge=(f'<span style="background:{bb};color:{bclr};padding:2px 7px;'
               f'border-radius:12px;font-size:9px;font-weight:600;'
               f'font-family:JetBrains Mono">{bf} {score}</span>')
    tag_html=("".join(f'<span style="background:{C3};color:{SUB};padding:1px 6px;'
              f'border-radius:4px;font-size:9px;font-family:JetBrains Mono">#{t}</span>'
              for t in tags[:2])) if tags else ""
...
<div style="background:{CARD};border:1px solid {BORD};border-left:3px solid {accent};
  border-radius:8px;padding:14px;height:190px;display:flex;flex-direction:column">
  <div style="display:flex;justify-content:space-between;gap:8px;margin-bottom:8px">
    <div style="font-size:12px;font-weight:600;color:{TXT};line-height:1.4;overflow:hidden;
      display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;flex:1">{title}</div>
    {badge}
  </div>
  <div style="font-size:11px;color:{SUB};line-height:1.5;flex:1;overflow:hidden;
    display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical">{body}</div>
  <div style="display:flex;justify-content:space-between;align-items:center;
    margin-top:8px;padding-top:8px;border-top:1px solid {BORD}">
...
def disc_card_new(d):
    ...
    return f"""<a href="{url}" target="_blank" style="text-decoration:none">
<div style="background:{CARD};border:1px solid {BORD};border-left:3px solid {B7};
  border-radius:8px;padding:14px;min-height:90px;
  display:flex;flex-direction:column;justify-content:space-between">
  <div style="font-size:12px;font-weight:600;color:{TXT};line-height:1.4;overflow:hidden;
    display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical">{title}</div>
  <div style="display:flex;justify-content:space-between;margin-top:10px;
    padding-top:8px;border-top:1px solid {BORD};
    font-size:9px;color:{MUT};font-family:JetBrains Mono">
...
# ── 탭2: 뉴스 ───────────────────────────────────────────────
with t2:
    news_data = load_news(); stocks_news = news_data.get("stocks",{})
    ordered = [(r["name"],stocks_news[r["name"]]) for _,r in df_sorted.iterrows()
               if r["name"] in stocks_news and stocks_news[r["name"]]]
    if not ordered:
        st.markdown(f'<div style="color:{SUB};font-size:13px;padding:20px">'
                    f'종목 뉴스 미수집. 내일 KST 07:00 자동수집됩니다.</div>',
                    unsafe_allow_html=True)
    else:
        for i,(name,articles) in enumerate(ordered):
            ...
            if avg_s is not None:
                ...
                tone = (f'<span style="background:rgba(56,139,253,.15);color:{tc2};'
                        f'padding:2px 8px;border-radius:12px;font-size:9px;'
                        f'font-weight:600;margin-left:8px">평균 {avg_s:.1f}</span>')
            st.markdown(f"""
...
with t3:
    ...
    if not discs:
        st.markdown(f'<div style="color:{SUB};font-size:13px;padding:20px">'
                    f'DART_API_KEY 등록 후 공시가 자동수집됩니다.</div>',
                    unsafe_allow_html=True)
    else:
        ordered_d = [(r["name"],discs[r["name"]]) for _,r in df_sorted.iterrows()
                     if r["name"] in discs and discs[r["name"]]]
        if not ordered_d:
            st.markdown(f'<div style="color:{SUB};font-size:13px;padding:20px">'
                        f'최근 14일 공시 없음.</div>', unsafe_allow_html=True)
        else:
...
<div style="background:{CARD};border:1px solid {BORD};border-radius:12px;
  ...
  <div style="font-size:14px;color:{SUB}">
    종목이 2개 이상 등록되고 가격 데이터가 수집된 후 확인 가능합니다
        ...
        def calc_sharpe(ticker, rf=3.5, days=90):
            ...
            ann=float(ret.mean()*252*100); vol=float(ret.std()*np.sqrt(252)*100)
        ...
        def badge(v, thresholds, fmt, reverse=False):
            ...
            lo,hi=thresholds
            clr=(UP if v<lo else (SUB if v<hi else DN)) if reverse else \
                (DN if v<lo else (SUB if v<hi else UP))
            return (f'<span style="color:{clr};font-weight:700;'
                    f'font-family:JetBrains Mono,monospace">{fmt.format(v)}</span>')

        # ── 리스크 테이블 ──────────────────────────────────
        st.markdown(f'<div style="font-size:14px;font-weight:600;color:{SUB};'
                    f'margin:1rem 0 10px">리스크 지표 요약</div>', unsafe_allow_html=True)

        risk_rows=""
        for i,pos in enumerate(sorted(positions,key=lambda x:x["value_krw"],reverse=True)):
            clr=HOLD_COLORS[i%len(HOLD_COLORS)]; t=pos["ticker"]; is_usd=pos["currency"]=="USD"
            beta=calc_beta(t,is_usd); vol=calc_vol(t); mdd_v=calc_mdd(t); sh=calc_sharpe(t)
            ...
            m_html  = (f'<span style="color:{DN};font-weight:700;font-family:JetBrains Mono">'
                       f'{mdd_v:.1f}%</span>') if mdd_v else f'<span style="color:{MUT}">—</span>'
  ...
  <td style="padding:.7rem 1rem;text-align:center">{sh_html}</td>
        ...
        TH_S=(f"padding:.6rem 1rem;font-size:10px;color:{MUT};font-weight:500;"
              f"text-transform:uppercase;letter-spacing:.04em;"
              f"border-bottom:1px solid {BORD};text-align:center")
        st.markdown(f"""
...
<div style="background:{C2};border:1px solid {BORD};border-radius:8px;
  padding:10px 14px;font-size:10px;color:{SUB};margin-top:8px;
  ...
  <span>⭐ <b style="color:{TXT}">샤프</b> &lt;0 손실 · 0~1 보통 · &gt;1 우수</span>
        ...
        # ── 상관관계 매트릭스 (블루/레드 팔레트) ─────────────
        st.markdown(f'<div style="font-size:14px;font-weight:600;color:{SUB};'
                    f'margin-bottom:10px">상관관계 매트릭스</div>', unsafe_allow_html=True)
        ...
        if len(ret_mat.columns)>=2 and len(ret_mat)>=10:
            ...
            fig_c.update_layout(
                paper_bgcolor=CARD,plot_bgcolor=CARD,
                height=max(280,len(corr)*60), margin=dict(l=8,r=60,t=8,b=8),
            ...
            if not np.all(np.isnan(arr)):
                ...
                st.markdown(f"""
<div style="background:{C2};border:1px solid {BORD};border-radius:8px;
  padding:10px 14px;font-size:11px;color:{SUB};margin-top:8px;
  display:flex;gap:16px;flex-wrap:wrap">
  <span>🔗 <b style="color:{TXT}">최고 상관:</b>
    {cols_list[hi_i[0]]} × {cols_list[hi_i[1]]}
    <span style="color:{UP};font-weight:700"> {arr[hi_i]:.2f}</span> → 같은 방향</span>
  <span>🔀 <b style="color:{TXT}">최저 상관:</b>
    {cols_list[lo_i[0]]} × {cols_list[lo_i[1]]}
    <span style="color:{DN};font-weight:700"> {arr[lo_i]:.2f}</span> → 분산 효과</span>
</div>""", unsafe_allow_html=True)
        ...
        # ── 롤링 변동성 (파랑 계열) ──────────────────────────
        st.markdown(f'<div style="font-size:14px;font-weight:600;color:{SUB};'
                    f'margin-bottom:10px">롤링 변동성 추이 (30일 연환산)</div>',
                    unsafe_allow_html=True)
        fig_v = go.Figure()
        ...
        fig_v.add_hline(y=20,line_dash="dash",line_color=SUB,line_width=1,
            annotation_text="20% 경계",annotation_font_color=SUB,annotation_font_size=9)
        fig_v.update_layout(
            paper_bgcolor=CARD,plot_bgcolor=CARD,height=260,
            ...
            yaxis=dict(showgrid=True,gridcolor=G,ticksuffix="%",
